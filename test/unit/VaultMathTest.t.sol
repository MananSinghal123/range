...
import {
    TickMath,
    LiquidityAmounts
} from "../../src/libraries/UniswapV3Math.sol";
...
contract VaultMathHarness {
    ...
    function computeMintSlippage(
        ...
        return
            VaultMath.computeMintSlippage(sqrtTwap, lo, hi, a0, a1, liq, bps);
    }
...
contract VaultMathTest is Test {
    ...
    function test_computeOptimalSwap_belowRange_swapsToken1() public view {
        ...
        (bool z, uint256 amt) = h.computeOptimalSwap(
            sqrtA - 1,
            sqrtA,
            sqrtB,
            5e7,
    ...
    function test_computeMintSlippage_appliesFloor() public view {
        (uint256 min0Full, uint256 min1Full) = h.computeMintSlippage(
            SQRTP,
            LO,
            HI,
            1e8,
            ...
            0,
            0
        ); // 0 bps → exact expected
        (uint256 min0Cut, uint256 min1Cut) = h.computeMintSlippage(
            SQRTP,
            LO,
            HI,
            1e8,
    ...
    function test_computeOptimalSwap_inRange_oneSided_swapsPartNotAll()
        public
        view
    {
    ...
    function test_computeOptimalSwap_inRange_balanced_swapsNearZero()
        public
        view
    {
        uint160 sqrtP = 265_651_043_793_156_983_656_969_817;
        ...
        (uint256 want0, uint256 want1) = h.amountsForLiquidity(
            sqrtP,
            sqrtA,
            sqrtB,
            1e18
        );
        (, uint256 amt) = h.computeOptimalSwap(
            sqrtP,
            sqrtA,
            sqrtB,
            want0,
        ...
        assertLt(
            amt,
            want0 / 1000 + want1 / 1000 + 1,
            "balanced input should barely swap"
        );
    }
}
